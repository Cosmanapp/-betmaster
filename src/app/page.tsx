        ...
        {view === 'analysis' ? (
          <>
            <form onSubmit={handleSearch} className="relative mb-8 group text-left">
              ...
              />
              <button type="submit" disabled={isSearching} className="absolute right-3 top-1/2 -translate-y-1/2 bg-emerald-500 text-black text-[10px] font-black px-4 py-2 rounded-xl uppercase hover:bg-white transition-colors disabled:opacity-50">
            ...
            <div className="space-y-4">
              <div className="max-h-[450px] overflow-y-auto pr-1 space-y-4 custom-scrollbar">
                {betSlip.map((m: any, idx: number) => (
                  <div key={idx} className="bg-slate-950 p-5 rounded-[32px] border border-slate-800 shadow-xl text-left">
                    <div className="flex justify-between items-center mb-4 text-left">
                      <input type="text" value={m.user_tip} onChange={(e) => setBetSlip(betSlip.map(x=>x.fixture.id===m.fixture.id?{...x, user_tip: e.target.value}:x))} className="bg-slate-900 px-4 py-2 rounded-xl text-emerald-400 font-black uppercase text-xs outline-none border border-emerald-500/20 w-32 focus:border-emerald-500 shadow-inner" />
                    ...
                    <div className="flex items-center justify-between bg-slate-900 p-3.5 rounded-2xl border border-slate-800 shadow-inner text-left">
              ...
              <div className="pt-6 border-t border-slate-800 space-y-5 text-left">
