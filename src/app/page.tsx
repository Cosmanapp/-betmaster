        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {matches.map((m: any, i: number) => (
            <div key={i} className="bg-slate-900/50 p-6 rounded-[30px] border border-slate-800">
              <div className="text-[10px] text-emerald-400 font-bold mb-2 uppercase">
              ...
              <div className="bg-black/40 p-3 rounded-xl border-l-4 border-emerald-500">
                <div className="text-emerald-400 font-bold text-xs">{m.ai_tip || 'Analisi'}</div>
                <div className="text-[10px] text-slate-400 italic">{m.ai_reason || 'In elaborazione...'}</div>
              </div>
            </div>
          ))}
          {matches.length === 0 && (
            <p className="text-slate-500 text-center col-span-full">Nessun match disponibile al momento.</p>
          )}
        </div>
      )}
    </div>
