...
export default function BettingDashboard() {
  ...
  const fetchAnalysis = async (query = '') => {
    ...
    try {
      ...
      // Mostriamo i match subito con un consiglio temporaneo
      const initialMatches = raw.map((m: any) => ({
        ...m,
        ai_tip: "Analisi...",
        ai_reason: "L'intelligenza artificiale sta elaborando i dati..."
      }));
      setMatches(initialMatches);
      setLoading(false);

      // 2. Tenta l'analisi AI in background
      try {
        ...
        {view === 'analysis' ? (
          <>
            <form onSubmit={handleSearch} className="relative mb-8 group text-left">
              ...
              />
              <button type="submit" disabled={isSearching} className="absolute right-3 top-1/2 -translate-y-1/2 bg-emerald-500 text-black text-[10px] font-black px-4 py-2 rounded-xl uppercase hover:bg-white transition-colors">
            ...
            {loading ? (
              <div className="py-24 text-center">
                ...
                <div className="text-emerald-500 font-black uppercase tracking-widest text-sm italic">Sincronizzazione Palinsesto...</div>
              </div>
            ) : (
                  ...
                  <div key={i} className="bg-slate-900/30 border border-slate-800 rounded-[32px] p-6 hover:border-emerald-500/30 transition-all shadow-xl flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-center mb-6">
                        <span className="text-[12px] font-black bg-slate-950 text-emerald-400 px-4 py-2 rounded-lg border border-white/5 uppercase">{m.league.name}</span>
                      ...
                      <div className="flex items-center gap-4 mb-8">
                        <div className="flex-1 flex flex-col items-center gap-3 text-center">
                          <img src={m.teams.home.logo} alt="" className="w-14 h-14 object-contain bg-white/5 p-2 rounded-full border border-white/5" />
                          <span className="text-[15px] font-black text-white uppercase tracking-tighter leading-tight">{m.teams.home.name}</span>
                        </div>
                        <div className="text-slate-800 font-black text-[10px]">VS</div>
                        <div className="flex-1 flex flex-col items-center gap-3 text-center">
                          <img src={m.teams.away.logo} alt="" className="w-14 h-14 object-contain bg-white/5 p-2 rounded-full border border-white/5" />
                          <span className="text-[15px] font-black text-white uppercase tracking-tighter leading-tight">{m.teams.away.name}</span>
                        </div>
                      </div>
                    </div>
                    <div className="bg-slate-950 p-5 rounded-3xl border-l-4 border-emerald-500 shadow-2xl">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-emerald-400 font-black text-[16px] uppercase italic tracking-wider">{m.ai_tip}</span>
                      </div>
                      <p className="text-[16px] text-white leading-snug font-semibold italic">"{m.ai_reason}"</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-6 text-left animate-in fade-in duration-500">
             <div className="flex justify-between items-center bg-slate-900 p-8 rounded-[40px] border border-slate-800 shadow-2xl">
              <h2 className="text-lg font-black uppercase text-emerald-500 italic">Profit Tracker</h2>
              ...
              <div key={bet.id} className={`p-6 rounded-[35px] border-2 transition-all ${bet.status === 'won' ? 'border-emerald-500/50 bg-emerald-500/5' : bet.status === 'lost' ? 'border-red-500/30 bg-red-500/5' : 'border-slate-800 bg-slate-900/40'}`}>
                <div className="flex justify-between items-center mb-6">
                  <div className="flex flex-col text-left">
                    ...
                    <p className="text-2xl font-black text-white italic uppercase">Vincita: € {bet.potentialWin}</p>
                  </div>
            ...
            <div className="space-y-4">
              {betSlip.map((m: any, idx: number) => (
                <div key={idx} className="bg-slate-950 p-5 rounded-[32px] border border-slate-800 shadow-xl">
                  <div className="flex justify-between items-center mb-4">
                    <input type="text" value={m.user_tip} onChange={(e) => setBetSlip(betSlip.map(x=>x.fixture.id===m.fixture.id?{...x, user_tip: e.target.value}:x))} className="bg-slate-900 px-4 py-2 rounded-xl text-emerald-400 font-black uppercase text-xs outline-none border border-emerald-500/20 w-32" />
                    <button onClick={()=>setBetSlip(betSlip.filter(x=>x.fixture.id!==m.fixture.id))} className="text-red-500/50 hover:text-red-500"><Trash2 size={20}/></button>
                  </div>
                  <div className="flex items-center gap-4 mb-5">
                    <img src={m.teams.home.logo} className="w-10 h-10 object-contain" />
                    <div className="flex-1 text-[14px] font-black text-white uppercase tracking-tighter">
              ...
              <div className="pt-6 border-t border-slate-800 space-y-5">
                <div className="bg-emerald-500 p-7 rounded-[35px] text-center shadow-xl shadow-emerald-500/20">
                  ...
                  <p className="text-4xl font-black text-black italic tracking-tighter">€ {(Number(betSlip.reduce((acc, m) => acc * (m.user_odds || 1), 1)) * stake).toFixed(2)}</p>
                </div>
                <button onClick={saveToArchive} className="w-full bg-white text-black font-black py-5 rounded-[30px] uppercase text-[11px] tracking-[0.2em] flex justify-center gap-3 hover:bg-emerald-400 transition-all"><History size={20}/> Salva</button>
              </div>
            </div>
          )}
        </div>
